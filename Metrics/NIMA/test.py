...
parser.add_argument('--test_images', type=str, default='test/', help='path to folder containing images')
...
def main(model):
    ...
    txt_name = './nima_results/result_' +'mean_std.csv'
    ...
    for i, img in enumerate(test_imgs):
        ...
        mean, deviation = 0.0, 0.0
    if means_list:
        mean_of_means = sum(means_list) / len(means_list)
        print(f'Mean of the means: {round(mean_of_means, 5)}')
    else:
        print('No images processed. Cannot compute mean of the means.')
    outfile.close()
